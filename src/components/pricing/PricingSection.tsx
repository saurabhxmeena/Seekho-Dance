...
export function PricingSection({ className }: PricingSectionProps) {
  ...
  const comparisonRows = [
    ...
    },
    {
      ...
      free: "Step 01 only",
      monthly: "Full Routine (All Steps)",
    },
    {
  ...
  return (
    <section className={cn("px-3.5 sm:px-6 lg:px-8 max-w-5xl mx-auto pt-6 sm:pt-12 pb-6 sm:pb-10", className)}>
      
      {/* 1. Section Header */}
      <div className="text-center max-w-2xl mx-auto space-y-1.5 sm:space-y-2 mb-8 sm:mb-12">
        <p className="text-[11px] sm:text-xs font-semibold tracking-widest uppercase text-neutral-500 dark:text-neutral-400">
          Seekho Studio Pass
        </p>

        <h2 className="text-2xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-neutral-950 dark:text-white leading-tight">
          Choose the plan that&apos;s right for you.
        </h2>

        <p className="text-xs sm:text-base text-neutral-500 dark:text-neutral-400 max-w-md mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 max-w-4xl mx-auto mb-10 sm:mb-14 items-stretch">
        ...
        <div className="rounded-2xl sm:rounded-3xl bg-neutral-100/70 dark:bg-[#1C1C1E] border border-neutral-200/80 dark:border-neutral-800 p-5 sm:p-8 flex flex-col justify-between shadow-xs">
          <div className="space-y-4 sm:space-y-5">
            ...
            <div className="pt-0.5 sm:pt-1">
              <div className="flex items-baseline gap-1">
                <span className="text-3xl sm:text-4xl font-bold tracking-tight text-neutral-950 dark:text-white font-sans">
                  ₹499
                </span>
          ...
          <div className="pt-5 sm:pt-6 text-center text-[11px] text-neutral-400 dark:text-neutral-500">
      ...
      {/* 3. Comparison Section (Unified Table for Web & Mobile) */}
      <div className="max-w-4xl mx-auto mb-10 sm:mb-14">
        <div className="text-center mb-5 sm:mb-8 space-y-1">
          <h3 className="text-xl sm:text-3xl font-bold tracking-tight text-neutral-950 dark:text-white">
            Compare Free vs. Monthly Pass
          </h3>
        ...
        <div className="rounded-2xl sm:rounded-3xl bg-neutral-100/60 dark:bg-[#1C1C1E] border border-neutral-200/80 dark:border-neutral-800 overflow-hidden shadow-xs">
          ...
          <div className="divide-y divide-neutral-200/70 dark:divide-neutral-800/70 text-xs sm:text-sm">
            {comparisonRows.map((row, idx) => (
              <div key={idx} className="grid grid-cols-12 px-3 sm:px-6 py-2.5 sm:py-3.5 items-center hover:bg-neutral-200/30 dark:hover:bg-neutral-800/40 transition">
                <div className="col-span-6 font-medium text-[11px] sm:text-sm text-neutral-950 dark:text-neutral-100 pr-1.5 leading-snug">
                  {row.feature}
                </div>

                <div className="col-span-3 text-center text-neutral-500 dark:text-neutral-400 text-[10px] sm:text-xs">
                  {typeof row.free === "boolean" ? (
                    row.free ? (
                      <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-neutral-950 dark:text-white mx-auto stroke-[2.5]" />
                    ) : (
                      <Minus className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-neutral-300 dark:text-neutral-600 mx-auto" />
                    )
                  ) : (
                    <span className="font-mono">{row.free}</span>
                  )}
                </div>

                <div className="col-span-3 text-center font-semibold text-neutral-950 dark:text-white text-[10px] sm:text-xs">
                  {typeof row.monthly === "boolean" ? (
                    row.monthly ? (
                      <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-orange-600 dark:text-orange-400 mx-auto stroke-[2.5]" />
                    ) : (
                      <Minus className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-neutral-300 dark:text-neutral-600 mx-auto" />
                    )
                  ) : (
                    <span className="font-mono text-orange-600 dark:text-orange-400 font-semibold">{row.monthly}</span>
                  )}
                </div>
              </div>
            ))}
